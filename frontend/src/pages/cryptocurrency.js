...
export const Cryptocurrency = (props) => {
  ...
  const checkData = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (resNode)
          getTxChainFrom(walletAddress); //있으면 db에서 데이터 가져옴
        ...
        for (let i = 0; i < second.length; i++) {
          for (let j = 0; j < first.length; j++) {
            if (second[i].length != 0 && first[j].data.to == second[i].from) {
              ...
              if (second[i].count > 1)
                (secondNode.address = secondNode.count),
                  (secondNode.tx = secondNode.count);
              const secondLink = {
                source: j + 2,
        ...
        if (nextNodes[i].count > 1) {
          for (let k = 0; k < nextNodes[i].count; k++) {
            const thirdNode = {
              ...
              name:
                "node" +
                (nextNodes.length + Node.length + 1) +
                "_node" +
                nextNodes[i].id,
              tx: nextNodes[i].tx[k],
  ...
  const onClickNode = function (nodeId, node) {
    ...
    function findChild(element) {
      if (element.parent == node.id) return true;
    }
