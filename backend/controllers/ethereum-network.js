...
const getEthSupplyCount = async (req, res) => {
  ...
  try {
    ...
    return cwr.errorWebResp(
      ...
      header,
      500,
      "Loading the number of ethereum supply failed",
      e.message || e
    );
  }
};
...
const getGasPriceStats = async (req, res) => {
  ...
  try {
    ...
    return cwr.errorWebResp(
      ...
      header,
      500,
      "Loading the statistic of gas price failed",
      e.message || e
    );
  }
};
...
const postTransactionInfo = async (req, res) => {
  ...
  try {
    ...
    if (blockInfo.transactions[0] !== null) {
      const transactionsCount = await req.web3.eth.getBlockTransactionCount(
        BlockNum
      );
      ...
      const transactionInfos = await Promise.all(
        blockNumbers.map((n) =>
          req.web3.eth.getTransaction(blockInfo.transactions[n])
        )
      );
      const filteredTxs = await Promise.all(
        transactionInfos.filter((transaction) => {
          if (transaction.to !== null) {
            return transaction;
          }
        })
      );
      const filteredTxInfos = await Promise.all(
...
const postEthAccountTraceRecord = async (req, res) => {
  ...
  try {
    ...
    const ethtraceCheck = await eth_account_traces.find({
    ...
    if (ethtraceCheck.length === 0) {
      ...
      eth_account_traces
        .insertMany(ethTraceRequest, { upsert: true })
        .catch((err) => {
          console.log(err);
        });
      return cwr.createWebResp(res, header, 200, {
    ...
    if (Number(ethtraceCheck[0].startBlockNumber) > Number(startBlockNum)) {
      eth_account_traces
        .updateMany(
          { startBlockNumber: ethtraceCheck[0].startBlockNumber },
          { $set: { startBlockNumber: startBlockNum, isUpdated: true } }
        )
        .catch((err) => {
          console.log(err);
        });
    ...
    if (Number(ethtraceCheck[0].endBlockNumber) < Number(endBlockNum)) {
      eth_account_traces
        .updateMany(
          { endBlockNumber: ethtraceCheck[0].endBlockNumber },
          { $set: { endBlockNumber: endBlockNum, isUpdated: true } }
        )
        .catch((err) => {
          console.log(err);
        });
    ...
    return cwr.errorWebResp(
      ...
      header,
      500,
      "Adding Ethereum Account Record failed",
      e.message || e
    );
  }
};
...
const postERC20TokenAccountTraceRecord = async (req, res) => {
  ...
  try {
    const { walletAddress, startBlockNum, endBlockNum } = req.body;
    const ERC20TokenTraceCheck = await ERC20Token_account_traces.find({
      address: walletAddress,
    });
    if (ERC20TokenTraceCheck.length === 0) {
      ...
      ERC20Token_account_traces.insertMany(ERC20TokenTraceRequest, {
        upsert: true,
      }).catch((err) => {
        console.log(err);
      });
      return cwr.createWebResp(res, header, 200, {
        message:
          "ERC20 Token Account Record loading Completed, database updated!",
      });
    }
    if (
      Number(ERC20TokenTraceCheck[0].startBlockNumber) > Number(startBlockNum)
    ) {
      ERC20Token_account_traces.updateMany(
        { startBlockNumber: ERC20TokenTraceCheck[0].startBlockNumber },
        { $set: { startBlockNumber: startBlockNum, isUpdated: true } }
      ).catch((err) => {
        console.log(err);
      });
    ...
    if (Number(ERC20TokenTraceCheck[0].endBlockNumber) < Number(endBlockNum)) {
      ERC20Token_account_traces.updateMany(
        { endBlockNumber: ERC20TokenTraceCheck[0].endBlockNumber },
        { $set: { endBlockNumber: endBlockNum, isUpdated: true } }
      ).catch((err) => {
        console.log(err);
      });
    ...
    return cwr.createWebResp(res, header, 200, {
      message:
        "ERC20 Token Account Record updating Completed, database updated!",
    });
  } catch (e) {
    return cwr.errorWebResp(
      res,
      header,
      500,
      "Adding ERC20 Token Account Record failed",
      e.message || e
    );
  }
};
...
const postTxlistChainWithAddress = async (req, res) => {
  ...
  try {
    ...
    const selectedTxlist = await Promise.all(
      txlist.result.filter((txReceipt) => {
        if (
          txReceipt.to !== walletAddress.toLowerCase() &&
          txReceipt.to !== "" &&
    ...
    // 목적지가 중복인 거래 목록 필터링
    const uniqueTxlist = await Promise.all(
      selectedTxlist.filter((addr, idx) => {
        return (
    ...
    const first_depth_tx = await Promise.all(
      uniqueTxlist.map((txReceipt) => {
        const txData = {
          tx: txReceipt.hash,
          data: {
            from: txReceipt.from,
    ...
    while (address_idx < to_addresses.length) {
      ...
      if (address !== "") {
        ...
        const filteredTxlist = await Promise.all(
          relatedTxlist.result.filter((txReceipt) => {
            if (
              txReceipt.to !== address.toLowerCase() &&
              txReceipt.to !== "" &&
              txReceipt.to !== undefined &&
              txReceipt.value !== "0"
            ) {
        ...
        const address_related_Tx = await Promise.all(
          filteredTxlist.map((relatedTxReceipt) => {
            if (
              relatedTxReceipt.to !== "" &&
              relatedTxReceipt.to !== undefined &&
              relatedTxReceipt.to !== relatedTxReceipt.from &&
              ...
              const txData = {
                ...
                data: {
                  ...
                  value: req.web3.utils.fromWei(
                    String(relatedTxReceipt.value),
                    "ether"
                  ),
                },
              };
    ...
    return cwr.errorWebResp(
      ...
      header,
      500,
      "get Transaction trace list failed",
      e.message || e
    );
  }
};
...
const postTokenTxChainWithAddress = async (req, res) => {
  ...
  try {
    ...
    const selectedTokenTxlist = await Promise.all(
      tokenTxlist.result.filter((tokentxReceipt) => {
        if (
          tokentxReceipt.to !== walletAddress.toLowerCase() &&
          tokentxReceipt.to !== "" &&
          tokentxReceipt.to !== undefined &&
    ...
    const uniqueTokenTxlist = await Promise.all(
      selectedTokenTxlist.filter((addr, idx) => {
        return (
    ...
    const first_layer_tokentx = await Promise.all(
      uniqueTokenTxlist.map((tokentxReceipt) => {
        const tokentxData = {
    ...
    while (address_idx < token_to_addresses.length) {
      ...
      if (address !== "") {
        ...
        const filtered_tokenTxlist = await Promise.all(
          related_tokenTxlist.result.filter((tokentxReceipt) => {
            if (
              tokentxReceipt.to !== address.toLowerCase() &&
              tokentxReceipt.to !== "" &&
              tokentxReceipt.to !== undefined &&
              tokentxReceipt.value !== "0"
            ) {
        ...
        const address_related_tokentx = await Promise.all(
          filtered_tokenTxlist.map((relatedTokenTxReceipt) => {
            if (
              relatedTokenTxReceipt.to !== "" &&
              relatedTokenTxReceipt.to !== undefined &&
              relatedTokenTxReceipt.to !== relatedTokenTxReceipt.from &&
              relatedTokenTxReceipt.value !== "0"
            ) {
              const tokentxData = {
                ...
                data: {
                  ...
                  value: req.web3.utils.fromWei(
                    String(relatedTokenTxReceipt.value),
                    "ether"
                  ),
                },
              };
    ...
    eth_tokentx_traces
      .insertMany(tokentxChain, { upsert: true })
      .catch((err) => {
        console.log(err);
      });
    return cwr.createWebResp(res, header, 200, {
      message:
        "Token transaction trace list loading Completed, database updated!",
    });
  } catch (e) {
    return cwr.errorWebResp(
      res,
      header,
      500,
      "get token transaction trace list failed",
      e.message || e
    );
  }
};
...
const postBlockInfo = async (req, res) => {
  ...
  try {
    ...
    let lastBlockNum = await ethBlocks
      .find({ network: req.body.endpoint })
      .sort({ blockNumber: -1 })
      .limit(1);
    if (lastBlockNum == "") {
      lastBlockNum = 0;
    } else {
      lastBlockNum = lastBlockNum[0]["blockNumber"];
    } // 마지막으로 저장된 blockNum 찾기
    if (startBlockNum < lastBlockNum) {
      startBlockNum = lastBlockNum + 1;
    } // DB에 데이터 없을 때 안전빵

    const blockNumbers = Array.from(
      { length: Number(endBlockNum) - Number(startBlockNum) + 1 },
      (v, i) => Number(startBlockNum) + i
    );

    if (blockNumbers != "") {
      const blockInfo = await Promise.all(
        blockNumbers.map((n) => req.web3.eth.getBlock(n))
      );
      const filteredBlockInfo = await Promise.all(
        blockInfo.map((block) => {
